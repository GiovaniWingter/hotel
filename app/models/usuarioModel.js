...
const usuarioModel = {
    ...
    create: async(valoresCampos)=>{
        try{
            ...
            pool.query("insert into usuario set ?", [valoresCampos]);
            return resultado;
        }catch(erro){
            console.log(erro);
            return null;
    ...
    update: async(valoresCampos, id)=>{
        try{
            const [resultado] = await 
            pool.query("update usuario set ? where id_usuario = ?", 
                [valoresCampos, id]);
            return resultado;
        }catch(erro){
            console.log(erro);
    ...
    deleteLogico: async (id)=>{ // delete lógico -> torna o registro inativo
        try{
            const [resultado] = await 
            pool.query("update usuario set status_usuario = 0  where id_usuario = ?",
                 [id]);
            return resultado;
        }catch(erro){
            console.log(erro);
